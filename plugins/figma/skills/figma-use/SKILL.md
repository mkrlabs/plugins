...
**If Figma MCP tools appear as deferred tools, batch-load all their schemas in a single `tool_search` call** using the `select:` syntax — e.g. `tool_search query="select:use_figma,get_screenshot,get_metadata,create_new_file"`. One round trip beats six.
