...
The Figma MCP tools (`use_figma`, `get_figjam`, `get_screenshot`, `get_metadata`, `create_new_file`, `whoami`) often appear as deferred tools that require `tool_search` to load their schemas before they can be called. **Load all schemas in a single `tool_search` call** using the `select:` syntax instead of one call per tool:

```
tool_search query="select:use_figma,get_figjam,get_screenshot,get_metadata,create_new_file"
```

Six sequential `tool_search` calls is six round trips before any work happens. One batched call is one round trip.
